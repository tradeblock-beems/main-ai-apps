...
export const replaceVariables = (text: string, userData: Record<string, any>): string => {
  return text.replace(/\[\[var:([a-zA-Z_][a-zA-Z0-9_]*)\]\]/g, (match, varName) => {
    ...
    return value !== undefined ? String(value) : match; // Keep original if no value found
  });
};
...
export const processVariableReplacements = (
  ...
  return csvData.map(userData => ({
    ...
    title: replaceVariables(title, userData),
    body: replaceVariables(body, userData),
    deepLink: deepLink ? replaceVariables(deepLink, userData) : undefined
  }));
}; 