...
def health():
    ...
    try:
        ...
        try:
            campaigns = load_campaign_data()
            health_status['campaign_data_available'] = len(campaigns) > 0
            health_status['campaign_count'] = len(campaigns)
        except Exception as e:
            health_status['campaign_data_available'] = False
            health_status['campaign_data_error'] = str(e)
        
        # Test CSV script discovery
        try:
            scripts = get_csv_scripts()
            health_status['csv_scripts_available'] = len(scripts) > 0
            health_status['csv_scripts_count'] = len(scripts)
        except Exception as e:
            health_status['csv_scripts_available'] = False
            health_status['csv_scripts_error'] = str(e)
...
def static_files(filename):
    """Handle static files for subpath deployment"""
